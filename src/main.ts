...
function getExperienceText(hobby: string, years: number): string {
  if (years > 5) return `Wow, you're a real expert in ${hobby}!`;
  if (years >= 1) return `Great, you already have experience in ${hobby}.`;
...
document.addEventListener("DOMContentLoaded", () => {
  const btn = document.getElementById("greetBtn") as HTMLButtonElement;
  ...
  const inputs = {
    ...
    hobby: document.getElementById("hobby") as HTMLInputElement,
  ...
  btn.addEventListener("click", () => {
    ...
    for (let key in BackgroundTheme) {
      if (isNaN(Number(key))) {
        const className = BackgroundTheme[key as keyof typeof BackgroundTheme];
        list += `
